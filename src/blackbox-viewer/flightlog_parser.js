 ...
 * (Cleanflight / iNav / Baseflight / Rotorflight / unknown) and anything
 * below the floor resolve to API_VERSION_1_44, which yields the base
 * definition tables.
 *
 * NOTE (RF decision, 2단계): Rotorflight debug modes are NOT mapped through
 * the shared Betaflight tables (`src/js/utils/debugModes.js`, generated from
 * betaflight/betaflight `debug.h`). RF keeps its own `DEBUG_MODE_RF_*`
 * tables inside `flightlog_fielddefs.js` (as the reference `rfblackbox`
 * does). So RF always resolves here to API_VERSION_1_44 (shared-table
 * fallback) and the presenter must prefer the fielddefs RF tables when
 * `firmwareType === FIRMWARE_TYPE_ROTORFLIGHT` (3/4단계에서 구현).
 *
 ...
 */
export function firmwareToApiVersion(firmwareType, firmwareVersion) {
...
export function FlightLogParser(logData) {
     ...
     * (RF behaves as modern firmware — reference rfblackbox treats ROTORFLIGHT as modern unconditionally)
     */
    const isModernFilterFirmware = () =>
        this.sysConfig.firmwareType === FIRMWARE_TYPE_ROTORFLIGHT ||
    ...
    const parseFirmwareRevision = (fieldValue) => {
        // Extract the firmware revision in case of Rotorflight/Betaflight/Raceflight/Cleanflight 2.x/Other
        // (RF reference: `/(.*flight).* (\d+)\.(\d+)(\.(\d+))*/i` — here kept as explicit alternation)
        const fwMatches = /((?:Beta|Race|Clean|Base|Butter|Rotor)flight)\s+(\d+)\.(\d+)(?:\.(\d+))?/i.exec(fieldValue);
        const FIRMWARE_NAME_MAP = {
            ...
            rotorflight: FIRMWARE_TYPE_ROTORFLIGHT,
        };
    ...
    const accelLimitHandler = (fn, fv) => {
        ...
        const isCfModern =
            this.sysConfig.firmwareType === FIRMWARE_TYPE_CLEANFLIGHT &&
            semver.gte(this.sysConfig.firmwareVersion, "2.0.0");
        // Reference rfblackbox: ROTORFLIGHT always takes the modern (/1000) branch.
        const isRf = this.sysConfig.firmwareType === FIRMWARE_TYPE_ROTORFLIGHT;
        this.sysConfig[fn] = isRf || isBfModern || isCfModern ? Number.parseInt(fv, 10) / 1000 : Number.parseInt(fv, 10);
    };
    ...
    const gyroScaleHandler = (_fn, fv) => {
        ...
        if (
            ...
            this.sysConfig.firmwareType === FIRMWARE_TYPE_CLEANFLIGHT ||
            this.sysConfig.firmwareType === FIRMWARE_TYPE_BETAFLIGHT ||
            // Rotorflight logs store gyro in deg/s with gyro_scale=1.0 like modern BF
            // (reference rfblackbox skips conversion because it has no IMU path,
            // but this viewer needs rad/us for updateEstimatedAttitude, so convert same as BF).
            this.sysConfig.firmwareType === FIRMWARE_TYPE_ROTORFLIGHT
        ) {
    ...
    HEADER_HANDLERS["Firmware type"] = (_fn, fv) => {
        // Reference (rfblackbox): "Rotorflight" -> ROTORFLIGHT, "Cleanflight" -> CLEANFLIGHT, else BASEFLIGHT.
        if (fv === "Rotorflight") {
            this.sysConfig.firmwareType = FIRMWARE_TYPE_ROTORFLIGHT;
        } else {
