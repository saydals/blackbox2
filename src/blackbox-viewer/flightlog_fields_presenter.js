...
// RF friendly field names (ref: rfblackbox/js/flightlog_fields_presenter.js:7-163, 123키 전량 이식).
// BF 테이블(FRIENDLY_FIELD_NAMES)과 분리 유지 — fieldNameToFriendly의 RF 분기에서만 우선 참조.
const FRIENDLY_FIELD_NAMES_RF = {
    "axisP[all]": "PID P",
    "axisP[0]": "PID P [roll]",
    "axisP[1]": "PID P [pitch]",
    "axisP[2]": "PID P [yaw]",
...
FlightLogFieldPresenter.fieldNameToFriendly = function (fieldName, debugMode, apiVersion, firmwareType, firmwareVersion) {
    ...
    if (firmwareType === FIRMWARE_TYPE_ROTORFLIGHT && FRIENDLY_FIELD_NAMES_RF[fieldName]) {
        return FRIENDLY_FIELD_NAMES_RF[fieldName];
    }
